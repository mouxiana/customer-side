<template>
  <div class="container mx-auto px-4 py-8">
    ...
    <div v-else-if="!userProfile" class="text-center py-12">
      ...
      <router-link to="/login" class="bg-primary text-white px-6 py-3 rounded-button hover:bg-orange-600">
        Login Now
      </router-link>
    </div>
    ...
    <div v-else class="flex flex-col lg:flex-row gap-8">
      <div class="lg:w-1/4">
        <div class="bg-white rounded-xl shadow-lg p-6">
          ...
          <nav class="space-y-2">
            <router-link to="/account" class="flex items-center p-3 text-primary bg-primary/10 rounded-lg">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <div class="px-4 py-3 border border-gray-200 rounded-lg bg-gray-50">{{ userProfile.full_name }}</div>
            </div>
            <div>
              <div class="text-sm text-gray-500 mb-1">Email</div>
              <div class="px-4 py-3 border border-gray-200 rounded-lg bg-gray-50">{{ userProfile.email }}</div>
            </div>
            <div class="md:col-span-2">
              <div class="text-sm text-gray-500 mb-1">Shipping Address</div>
              <div class="px-4 py-3 border border-gray-200 rounded-lg bg-gray-50 whitespace-pre-line">{{ userProfile.shipping_address || 'No shipping address' }}</div>
            </div>
          </div>
        </div>
